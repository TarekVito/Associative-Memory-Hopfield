function [TestIn,TestRes,DataSet] = PrepareData()
...
Data = zeros(usersSize,uniqAllMSize);
for i=1:usersSize
    userIdx= users(i);
...
for i=1:size(TestIn,1)
    ...
    %curUser(oneIdx)=0;
    rndP = randperm(size(curUser,2));
    rndP = rndP(1:round(size(rndP,2)/10));
    curUser(rndP) = 0;%-1+round(rand(size(rndP)))*2;
